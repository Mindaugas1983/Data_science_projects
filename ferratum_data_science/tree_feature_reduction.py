...
import ferratum_data_science.algorithms as alg
...
def decreasing_crossvalidation(origin_model, cv_model, train_origin, target, itype = 'weight'):
    ...
    '''
    This function at prints cross-validation metrics of  xgb model trained with increasing number of features ordered by importance. Beginning with one feature and increasing feature count by 1 
    Parameters:
    origin_model: xgb model which was used to extract feature importance
    cv_model: model for producing cross-validation results
    train_origin: dataframe with all features
    target: series with model target
    itype: xgboost feature importance type - 'weight', 'gain' or 'cover'
    '''
