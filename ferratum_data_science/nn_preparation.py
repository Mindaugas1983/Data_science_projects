...
class MultivariateTimeseriesBatchGenerator(object):
    ...
    '''
    This class provides a generator for observations with different history length of time-series for neural network (LSTM or ) training
    
    Parameters of constuctor:
    ...
    def __init__(self, data, feature_count, target_name, client_identificator, max_history_length):
        ...
        self.batch_size = 1
        self.current_idx = 0
        self.skip_step = data[data[client_identificator] == data.iloc[0][data.columns.get_loc(client_identificator)]].shape[0]
    ...
    def generate(self):
        while True:
            if self.current_idx + self.max_history_length >= len(self.data):
                    self.current_idx = 0
            self.skip_step = self.data[self.data[self.client_identificator] == self.data.iloc[self.current_idx][self.data.columns.get_loc(self.client_identificator)]].shape[0]
