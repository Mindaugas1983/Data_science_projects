...
def save_observation_list(data, path, name):
    ...
    if os.path.exists(path):
        ...
        observations = pd.DataFrame({"Target": ["BAD","GOOD","TOTAL"], "Total": [amounts[0],amounts[1],amounts[1] + amounts[0]], "Percentage": [ round(amounts[0] / ( amounts[1] + amounts[0] ) * 100, 2) , round(amounts[1]/(amounts[1] + amounts[0]) * 100, 2) , "-"]})
...
def get_NN_feature_effects(test_X, test_app, n_features, shap_explainer , id_column = 'application_id'):
    ...
    for i in range(len(test_X)):
        ...
        if n_features == -1:
            ...
            cnt = n_features
        first_row_effects_list = dc.calculate_top_contributors(shap_values=np.asarray(shap_values).reshape(cnt), features=test_X.iloc[i], feature_names=list(test_X.columns), return_df =True, n_features=n_features, use_abs=True)
